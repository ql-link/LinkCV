...
评论正文展示新增、修改、删除和验收变化；固定标题、原需求指纹、变更 ID、时间、评论 ID、内容哈希和替代关系全部由工具生成或记录，开发者不填写 `state.yaml`，也不维护任何标识。命令成功并复核评论进入权威需求链后，Agent 删除临时输入文件。
...
结构化评论采用追加式审计：旧评论不编辑。若后来纠正一条已确认评论，Agent 根据当前有效评论链自动选择被替代评论，并通过工具参数追加新评论；`supersedes` 只属于工具元数据，不要求开发者判断或输入。任何普通评论，即使讨论了范围，也仍不自动成为权威需求；需要进入权威需求时必须重新形成业务差异、获得确认并追加结构化评论。
