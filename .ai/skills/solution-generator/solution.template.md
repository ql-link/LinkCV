...
| 一句话需求 | <一句话说明本次要交付什么> |
| 来源材料 | <当前请求、Issue 链接或用户明确指定的外部材料；Issue 为可选追踪信息> |
| 复杂度 | <简单 / 中等 / 复杂；写主导原因> |
| 风险 | <常规 / 严格；严格时写具体触发项> |
| 记录 | <持久记录的原因> |
...
- **判断依据**：<当前请求、Issue、用户指定的外部材料、其他已确认材料或当前产品行为>
...
<使用正常文本按数据对象说明变更层级、读取方、写入方、归属与权限，以及必须保持的不变量；不使用横向表格。涉及多个数据对象时分别使用五级标题。>

- **模型定义现状**：<真实 ORM 或模型事实；不改结构时也要核对>
- **仓库迁移 head**：<修改结构或迁移数据时填写>
- **目标环境 current revision**：<已核实值；无法核实时明确写“尚未核实”>

<只使用现有数据且不修改结构时，填写本节后删除 7.2 和 7.3。新增或修改表、字段、约束、索引、枚举时继续填写 7.2；只有存在存量数据处理、兼容窗口、迁移顺序、数据回退或不可逆点时才保留 7.3。>

#### 7.2 数据结构变更

<按发生变化的表分别填写。字段、新表、主键、约束和索引使用 SQL 风格的结构定义片段，不使用字段表格。结构定义片段只供开发评审，不是可直接执行的 DDL，不生成 CREATE TABLE、ALTER TABLE、INSERT、UPDATE、DELETE 等完整语句。>

##### 实体关系（按需）

<只有多个实体或关联发生变化时保留本段；没有关系变化时删除，不影响后续编号。>
...
<新增表展示全部字段；已有表按实际变化拆分标题。字段代码块只写字段，不混入主键、外键、检查约束或索引。没有对应变化的标题和代码块直接删除。>

```sql
field_name varchar(64) not null comment '字段含义'
```

业务约束：

- <只写影响数据正确性或实现方式的约束；没有则删除本段>

约束变更：

```sql
primary key pk_example (id)
foreign key fk_example_owner (owner_id) references users (id)
check ck_example_status
```

<约束有变化时才保留，逐项用正常文本说明约束语义；字段代码块中不重复。>

索引变更：

```sql
idx_example (owner_id, created_at)
```

<索引有变化时才保留，逐项说明支撑的真实查询；字段代码块中不重复。>

#### 7.3 存量数据、兼容与迁移

> 条件章节。只在存在存量数据回填或清理、兼容窗口、迁移顺序、数据回退或不可逆点时保留；全部未命中时删除整节。

<使用正常文本描述存量数据的处理范围、回填规则和异常数据结果，不输出可执行 SQL。>
...
| --- | --- | --- | --- | --- | --- |
| D1 |  |  |  |  | 当前请求 / 用户确认 / 用户指定的外部材料 / 来源 Issue |
