---
name: run-all-tests
description: 根据 LinkCV 的实际改动范围选择并运行验证命令，区分本次命令产生的代码失败与环境阻塞，报告真实命令、结果和未覆盖区域。适用于实现完成后的验证、复现并初步归因测试命令失败以及提交 PR 前的完整检查；用户只给出现象或日志并要求调查跨服务根因时使用 incident-triage，L2/L3 验证成功后把证据写入本地规格状态。
---
