---
name: run-all-tests
description: 根据 LinkCV 的实际改动范围选择并运行验证命令，区分本次命令产生的代码失败与环境阻塞，报告真实命令、结果和未覆盖区域。适用于实现完成后的验证、复现并初步归因测试命令失败以及提交 PR 前的完整检查；用户只给出现象或日志并要求调查跨服务根因时使用 incident-triage，不维护本地规格状态或验证快照。
---
...
持久设计和直接实现使用同一套真实命令。后端与混合方案任务执行前读取 `solution.md` 的“验证与验收”，纯前端持久设计任务读取 `ui-design.md` 的验证计划，混合任务存在 UI 附件时同时读取且不允许 UI 验证改写上游契约；直接实现读取当前请求、来源材料和已确认结果。所有路径都要结合当前实现、Git 差异和实际风险，选择一条或多条必要命令直接运行；不创建验证状态文件，不把较早会话中的“已通过”当作当前代码证据。

代码或配置在验证后发生变化时，由 AI 根据真实影响重跑受影响检查；无法可靠缩小影响时扩大验证。创建 PR 时无论之前跑过哪些局部命令，都在当前可提交内容上重新运行完整 `npm run check`。
