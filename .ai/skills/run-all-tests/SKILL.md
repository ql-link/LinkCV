...
- **任务范围验证**：任务完成时运行与改动范围和风险匹配的检查；方案先行任务把所选命令绑定到当前代码快照。
- **PR 全量验证**：准备创建 PR 时，在当前可提交内容上运行完整 `npm run check`；共享 CI 继续运行同一完整入口。

修复阶段先运行最窄且有区分度的检查，再扩大到本任务需要的范围。任务范围验证不因为“最终验证”自动变成全仓检查，PR 全量验证也不能被较早的局部结果替代。
...
修复和开发过程中可以直接运行窄检查；进入任务范围验证时，由阶段工具亲自执行已经按改动范围和风险选定的命令，并自动记录退出码、耗时、时间和当前可提交内容指纹。它记录的是本任务选择的验证范围，不等于 PR 全量验证。仅修改 AI 规则时可以使用：

```bash
npm run spec -- verify <KEY> --run "npm run check:ai"
```

如果前端行为需要多个必要命令，可以重复提供 `--run`：
...
脚本逐条执行，不接受手填的“已通过”证据；任一命令失败、命令执行期间改变了可提交内容或仍有未解决失败时都不会标记已验证。成功后阶段自动进入 `quality_review`，表示当前代码已经通过记录范围的验证，不直接表示可以创建 PR。

如果存在跨端、浏览器、上传下载、PDF 或视觉行为需要人工验收，本技能先报告所需自动化命令和未覆盖区域，转 `manual-acceptance`。人工验收全部通过后，由该技能运行 `spec verify --run "<任务范围自动化命令>" --manual-acceptance`；工具自动校验人工验收结论和文件哈希。开发者不需要填写哈希、退出码或时间。
...
- 准备创建 PR 时，完整 `npm run check` 已在当前可提交内容上通过；未通过或环境阻塞时不创建 PR；
