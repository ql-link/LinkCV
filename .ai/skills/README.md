...
| 技能 | 职责 | 边界或产出 |
| --- | --- | --- |
| `flow-router` | 在存在后端或跨端契约改动时区分纯后端和混合领域，并原样传递后端控制项 | 转 `backend-delivery`；明确的纯 Web 前端任务不使用 |
| `backend-delivery` | 当前对话的 Sol（主 Agent）完成纯后端和混合任务的七维判断、方案、工作包拆分、Luna 调度和实施复核，不引入同级 Sol | 复用 `solution-generator`、可选验收契约和 `implementation-execution`；混合任务的 UI 由当前 Codex 直接实现 |
...
用户当前请求、Issue、飞书文档及用户明确指定的其他材料都可以作为来源。Issue 存在时必须完整读取，但没有 Issue 不阻止开发，也不算例外。明确的纯 Web 前端任务由当前 Codex 直接处理，不先分类、不进入统一交付 Skill；只有当前问题需要时，才独立使用前端原型、浏览器核对或视觉对照能力。纯后端和混合任务继续通过 `flow-router` 进入现有 `backend-delivery`，复杂后端任务仍进入 `solution.md` 方案链，混合任务先固定跨端契约。

飞书文档只作为方案形成前的初始输入。确认后的 `solution.md` 是后端和混合方案的实施依据，保存业务、API、权限、数据与失败语义；纯前端只有在实际存在视觉材料时才以最新确认的 Figma、截图或原型为依据，不生成本地设计 Spec 或 Agent 交接文档。混合任务的 UI 实现只消费并从属于 `solution.md`；契约变化必须先改 `solution.md`。
...
后端/混合：flow-router → backend-delivery（Sol 七维判断）
                              ├── 直接实现 ───────────────────────┐
                              └── solution.md → 用户确认          │
                                               → 可选契约验收 ────┤
                                                                 ↓
...
后端主链只向右推进。纯前端没有主链，由当前 Codex 根据当前问题自由组合能力；实现优先复用现有组件和 Token，只有会改变用户结果或跨端契约的缺口才返回用户。`docs/` 是代码实现后的长期事实旁路，只在真实架构、接口、数据、配置或部署事实改变时同步，不参与任务方向裁决。

## 后端 Sol 调度与 Luna 工作包

后端与混合任务仍由 Sol 负责规划、拆分、调度、整合和复核。Sol 根据已确认的 `solution.md`、依赖关系、写冲突和文件边界决定实施队列：可独立、边界清楚且文件所有权不重叠的工作包可以并行交给多个 Luna；共享契约、迁移链、同一核心文件或存在前后依赖的工作包必须串行。纯前端不使用这套工作包调度，也不设置替代性的前端编排者。
...
后端和混合方案任务的需求确认权归 `solution-generator`。纯前端普通实现不调用项目交付 Skill；只有存在真实视觉问题时才使用 `frontend-prototype`，并由用户决定是否需要 Figma 和人工确认。混合任务不得在 UI 阶段重新决定上游契约。
