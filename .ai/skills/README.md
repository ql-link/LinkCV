...
用户当前请求、Issue 及用户明确指定的外部材料都可以作为来源。Issue 存在时必须完整读取，但没有 Issue 不阻止开发，也不算例外。项目 Skill 不主动创建、更新或回写外部规划文档；用户明确要求操作某个外部系统时，临时使用对应的全局能力，不把它加入项目交付链。明确的纯 Web 前端任务由当前 Codex 直接处理，不先分类、不进入统一交付 Skill；只有当前问题需要时，才独立使用前端原型、浏览器核对或视觉对照能力。只要存在或可能存在后端改动，就直接进入唯一后端入口 `flow-router`；它在同一上下文中确认范围并继续七维判断，复杂后端范围进入 `solution.md` 方案链，混合任务先固定跨端契约。后端链不承接前端实现或检查。

用户指定的外部材料只作为方案形成前的初始输入。确认后的 `solution.md` 是后端和混合方案中后端范围的实施依据，保存业务、API、权限、数据、失败语义与消费方可观察契约；它不记录或调度 UI 实现。纯前端只有在实际存在视觉材料时才以最新确认的 Figma、截图或原型为依据，不生成本地设计 Spec 或 Agent 交接文档。前端实现需要改变上游契约时必须先修订 `solution.md`。
