...
| 技能 | 职责 | 边界或产出 |
| --- | --- | --- |
| `flow-router` | 唯一后端开发入口：确认后端范围，并由当前 Codex 在同一上下文完成七维判断、路径选择、方案、直接实施和复核 | 复用 `solution-generator`、可选验收契约和 `implementation-execution`；明确的纯 Web 前端任务不使用，也不实现 UI 或编排前端能力 |
...
用户当前请求、Issue 及用户明确指定的外部材料都可以作为来源。Issue 存在时必须完整读取，但没有 Issue 不阻止开发，也不算例外。项目 Skill 不主动创建、更新或回写外部规划文档；用户明确要求操作某个外部系统时，临时使用对应的全局能力，不把它加入项目交付链。明确的纯 Web 前端任务由当前 Codex 直接处理，不先分类、不进入统一交付 Skill；只有当前问题需要时，才独立使用前端原型、浏览器核对或视觉对照能力。只要存在或可能存在后端改动，就直接进入唯一后端入口 `flow-router`；它在同一上下文中确认范围并继续七维判断。任务规模和复杂度只影响实施与验证，不自动创建 `solution.md`；只有关键取舍、协同发布、新长期边界、难回退结果或用户明确选择方案先行时才进入方案链。后端链不承接前端实现或检查。

用户指定的外部材料只作为方案形成前的初始输入。确认后的 `solution.md` 是后端和混合方案中后端范围的实施依据，保存业务、API、权限、数据、失败语义与消费方可观察契约；它不记录或调度 UI 实现。纯前端只有在实际存在视觉材料时才以最新确认的 Figma、截图或原型为依据，不生成本地设计 Spec 或 Agent 交接文档。已有方案的前端实现需要改变上游契约时先修订 `solution.md`；直接实现任务发现新方案门槛时返回 `flow-router` 重评。

## 单向交付层次

| 层次 | 载体 | 负责什么 | 边界 |
| --- | --- | --- | --- |
| 外部输入层 | 用户明确指定的外部材料 | 提供任务背景、约束和供人讨论的初步结论 | 项目工作流不负责创建或回写，进入开发链后不裁决实现 |
...
后端范围由当前 Codex 在同一上下文中负责规划、实施、验证和复核；混合任务只把后端、跨端契约、配置、迁移和部署范围纳入这里。当前 Codex 根据已确认的 `solution.md`、依赖关系、写冲突和文件边界确定实施顺序：共享契约、迁移链、同一核心文件或存在前后依赖的改动必须按依赖完成。默认不创建实施子 Agent、不切换模型，也不为了并行拆分任务；纯前端同样不设置替代性的前端编排者。

实施前写清目标、可观察结果、允许与禁止文件、依赖、严格检查和最小验证。当前 Codex 复用已经读取的代码上下文连续完成修改；确认范围内的问题直接修正，新的产品、权限、数据、兼容、迁移或发布决定按归属回流，不通过 Agent 交接重新读取和规划。只有用户明确要求独立审查或并行 Agent 工作时，才按该次授权使用子 Agent。

普通文件拆分、命名、测试落点或不改变外部结果的实现调整，直接在代码中完成并按需写入 PR。已有方案任务需要改变用户行为、权限、数据语义、状态、兼容、迁移或发布承诺时修订当前 `solution.md`；直接实现任务只有新证据命中方案门槛时才转入方案链，否则继续按已明确结果实施。纯前端存在已确认视觉依据时服从其最新版本，混合任务同时服从已存在的上游契约。
...
技能调用保持单向且由上层拥有产物：`solution-generator` 可以按需调用 `decision-grilling` 处理单个选择，只有 `confirmed` 才写入结论，`blocked` 暂停，`replan` 交回上层重建决策树；被调用能力不接管完整方案或外部写入。

后端和混合方案中业务与后端契约的确认权归 `solution-generator`。它不确认 UI 设计，也不选择前端能力。纯前端普通实现不调用项目交付 Skill；只有存在真实视觉问题时才使用 `frontend-prototype`，并由用户决定是否需要 Figma 和人工确认。已有方案的混合任务若前端实现需要改变上游契约，则重新进入后端方案确认；直接实现任务先由 `flow-router` 按方案门槛重评，不能由 UI 阶段静默决定。
...
前端实现后根据实际差异选择检查：自动化验证使用 `run-all-tests`，运行时问题使用 `frontend-browser-check`，存在设计基准时使用 `frontend-visual-check`，代码审查仅在用户要求或风险确有必要时使用。它们互不构成前置条件。后端实施中只在能为下一步提供有效反馈的稳定节点运行最窄检查，完整差异稳定后统一执行一次任务级验证。PR 按实际领域组合检查；同一会话、未变化代码快照的充分证据可以复用，只有差异覆盖全仓、无法缩小范围或用户明确要求时运行完整 `npm run check`，共享 CI 保持独立。
