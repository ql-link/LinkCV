...
- 用户未指定交付方式时，改代码请求先由 `flow-router` 只判断纯前端、纯后端或前后端混合领域；纯前端由 `frontend-delivery` 选择交付档位，纯后端和混合任务由 `backend-delivery` 完成七维判断并选择后端路径。详细算法只在对应 Skill 维护。
...
- `直接实现` 不创建 Spec。纯后端和混合任务需要方案时先确认 `solution.md`，只有选择契约验收时才增加 `acceptance.feature`；纯前端由 `frontend-delivery` 选择轻量、标准或完整档。标准和完整档按“每轮一个高影响问题、一次可见修改、一次用户反馈”逐步收敛；已有可运行页面默认修改真实路由，全新页面使用隔离的 HTML/CSS 原型，用户明确指定时使用 Figma。用户明确要求收口后才统一完成验证与交付。
- `.specs/<KEY>/` 中纯后端与混合方案以 `solution.md` 为中心；前端以用户确认的真实路由当前差异、HTML 原型或 Figma 为视觉依据，已有页面不为预览重复创建 Spec，也不要求 `ui-design.md`。混合任务的视觉依据只从属 `solution.md`，不得改写业务、API、权限、数据或失败语义。所有路径都不保存阶段状态、哈希或历史测试标记，开始或恢复时重新读取当前请求、适用方案或视觉依据、Git 差异和真实代码；跨会话不继承旧测试结论。
...
- `docs/` 只描述已经实现的长期事实；后端方案、前端 HTML 原型、Acceptance、实施报告和人工验收记录放在 `.specs/<KEY>/`，用户指定的 Figma 设计保留在 Figma。
