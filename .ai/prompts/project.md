...
- 前端开发中发现必须改变 FastAPI、数据库、公共契约、权限、共享配置或部署行为时，停止纯前端假设，使用 `flow-router` 判断纯后端或前后端混合，再进入现有 `backend-delivery`。后端交付结构本轮不变。
- 用户当前请求、可读取的 Issue、飞书文档或其他指定材料都可作为需求来源；没有 Issue 不阻止开工。

以下控制项只适用于纯后端和前后端混合任务：
...
- 等价的自然语言同样有效。`flow-router` 原样传递后端控制项；`自动` 由 `backend-delivery` 判断，`开启` 使用用户已选路径，未选时仍由后端交付 Skill 判断；`后续：自动` 在确认方案时一并确定。`关闭` 直接执行。
- 用户更具体、更新的选择优先并立即生效；仍无法消解冲突时优先关闭工作流。自动判断不得静默覆盖用户选择或反复确认，最多简短提示一次真实风险。
- `直接实现` 不创建 Spec。纯后端和混合任务需要方案时先确认 `solution.md`，只有选择契约验收时才增加 `acceptance.feature`。
- 后端和混合方案继续以 `solution.md` 为中心，前端能力积木不得改写其中已经确认的业务、API、权限、数据或失败语义。
...
- 后端任务的 `工作流：关闭` 只跳过项目交付文档和分流，不跳过真实代码核实、必要验证、安全底线或 Git 授权边界。纯前端不解释这个控制项，因为它本身没有项目流水线。
