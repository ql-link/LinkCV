...
共有三条端到端路径：`直接实现`、`方案 → 实现`、`方案 → 验收契约 → 实现`。前一个岔口由 `flow-router` 判，后一个岔口在方案文档冻结时由 `solution-generator` 和用户一起定。
...
- 方案文档第一部分面向决策者，用白话写业务，不出现类名、方法名、表名、字段名、框架名和文件路径；通用技术术语直接使用不作解释，只有自创说法才在首次出现处就地说清。第二部分面向实现者，必须落到真实文件路径和按步骤编号的实现方案。方案文档冻结时必须和用户确认后续走直接施工还是契约验收，并把结论随 `--next` 一起记入状态。
...
- 整个模块的目标或商业前提尚未成形，或需要把结论沉淀到飞书供他人协作时，才使用 `module-planning` 完成事实调查、草稿确认、写入和读回；它按需调用 `decision-grilling`，每轮只处理一个需要用户取舍的决策。已有经确认并读回的飞书详情文档时，其结论优先于本地推断，冲突时回 `module-planning` 更新后再修订本地产物。
...
- 普通团队开发从一条来源 Issue 开始，必须完整读取其正文和关联材料。Issue 可以来自 Multica、Linear、GitHub 或其他系统；来源平台只作为关联元数据，不改变分流、规格或实现规则，也不得把项目流程绑定到某个平台。Issue 正文承担初始产品目标、现状和验收输入；内容已经足够明确且判为直接实现的任务可以直接据此实现，方案先行任务由 `solution-generator` 把 Issue、已有飞书详情文档和真实代码收敛到本地方案文档，其中的不确定点在该阶段直接向用户确认。没有外部 Issue 的用户直达请求只作为明确关闭工作流、自举或本地维护等例外处理。
...
- 实现阶段出现无法预见但不改变核心产品目标的偏差时，记录原因、影响、验证和遗留风险，并在 PR 中明确提出；不得为此自动修改来源 Issue。会改变核心范围、验收、权限、数据安全或兼容承诺的事项先回 `flow-router` 重评准备程度、复杂度、风险和记录需要；判为方案先行后由 `solution-generator` 重新确认并冻结产物。相关结论已经沉淀在飞书详情文档时，才先经 `module-planning` 更新并读回，再按顺序更新本地产物。
