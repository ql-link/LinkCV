...
- 用户未指定交付方式时，改代码请求先由 `flow-router` 判断纯前端、纯后端或前后端混合领域，再判断准备程度、复杂度、风险和记录需要；详细算法只在对应 Skill 维护。
...
- `直接实现` 不创建 Spec。纯后端和混合任务需要方案时先确认 `solution.md`，只有选择契约验收时才增加 `acceptance.feature`；纯前端由 `frontend-delivery` 选择轻量、标准或完整档，轻量不建 Spec，标准按需、完整必须创建 `ui-design.md`。
- `.specs/<KEY>/` 按领域选择中心：纯后端与混合方案以 `solution.md` 为中心，纯前端持久设计以 `ui-design.md` 为中心；混合任务的 `ui-design.md` 只从属 `solution.md`，不得改写业务、API、权限、数据或失败语义。所有路径都不保存阶段状态、哈希或历史测试标记，开始或恢复时重新读取当前请求、适用 Spec、Git 差异和真实代码；跨会话不继承旧测试结论。
...
- `docs/` 只描述已经实现的长期事实；后端方案、前端 UI 设计、Acceptance、实施报告和人工验收记录放在 `.specs/<KEY>/`。
