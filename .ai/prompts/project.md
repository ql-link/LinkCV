...
- 自动路径不按工作量或复杂度决定是否写方案。只有存在会改变业务结果、API、权限、数据语义、兼容、失败行为或发布承诺的关键取舍，或者需要协同迁移与发布、引入新的长期技术边界、处理难回退结果时，才创建 `solution.md`；目标明确且沿用现有边界的新增模块、多文件改动和普通增量 schema 变更直接实现。
...
- 修复或功能交付完成后，须启动本地栈并通过隧道（如 `cloudflared tunnel --url` 指向 Vite 端口）向用户提供可访问的预览 URL 与测试账号；功能效果由用户自行验收，不代替用户做端到端验收。
