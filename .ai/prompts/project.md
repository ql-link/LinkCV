...
- 默认情况下，改代码请求先由 `flow-router` 按真实影响面建议走直接实现还是方案先行；用户没有显式指定交付方式时，才按该建议和对应 Skill 推进。
...
- `后续：直接施工`：方案文档冻结后直接实现，不产出 `acceptance.feature`；方案文档 9.3 的验证方式即验证契约。
...
- 方案先行任务的需求与方案在同一阶段由 `solution-generator` 定稿：它完成模块分解、业务流程、状态机、整体架构与选型取舍、可落库数据模型、接口契约、文件结构与模块实现方案和实施顺序。它是与用户交互确认的核心阶段，业务规则和技术取舍的不确定点都在此直接提问，一次一个问题并给出推荐答案，结论写入方案文档的决策记录。
...
- 自动分流时，设计 MySQL 表结构使用 `mysql-ddl-conventions`，由方案文档阶段调用以定稿数据模型；编写、校验或排查 SQLAlchemy/Alembic 迁移使用 `alembic-migration`，在实现阶段按已冻结的定稿 DDL 落成配对 SQL。数据库改动默认判为方案先行。用户显式关闭工作流或指定其他路径时采用用户选择，但仍需执行与数据库风险相匹配的目标校验、迁移验证和回滚检查。
...
- 实现阶段出现无法预见但不改变核心产品目标的偏差时，记录原因、影响、验证和遗留风险，并在 PR 中明确提出；不得为此自动修改来源 Issue。会改变核心范围、验收、权限、数据安全或兼容承诺的事项返回 `solution-generator` 重新确认并修订冻结产物；结论已经沉淀在飞书详情文档时，先经 `module-planning` 更新并读回，再按顺序更新本地产物。
