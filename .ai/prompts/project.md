...
- 用户未指定交付方式时，改代码请求先由 `flow-router` 只判断纯前端、纯后端或前后端混合领域；当前对话的 Sol（主 Agent）通过 `frontend-delivery` 或 `backend-delivery` 完成判断、方案、工作包拆分、Luna 调度和实施复核，不引入同级 Sol。确认后的实施可交给一个或多个 Luna Max；Sol 根据 `solution.md`、依赖、写冲突和文件边界决定并行或串行，不为了并行而拆分。详细算法只在对应 Skill 维护。
...
- 等价的自然语言同样有效。`flow-router` 原样传递控制项；`自动` 由对应的 `frontend-delivery` 或 `backend-delivery` 判断，`开启` 使用用户已选路径，未选时仍由领域交付 Skill 判断；`后续：自动` 在确认方案时一并确定。`关闭` 直接执行。
- 用户更具体、更新的选择优先并立即生效；仍无法消解冲突时优先关闭工作流。自动判断不得静默覆盖用户选择或反复确认，最多简短提示一次真实风险。
- `直接实现` 不创建 Spec。纯后端和混合任务需要方案时先确认 `solution.md`，只有选择契约验收时才增加 `acceptance.feature`；纯前端由 `frontend-delivery` 判断非视觉直接修改或原型驱动实现，当前对话 Sol 核对用户原型、真实代码和行为契约。目标明确、局限在现有组件且不新增交互状态、API、共享契约或跨文件协调的低风险局部修改由 Sol 直接完成；具有实质实施量的工作包才通过 `frontend-implementation` 交给一个或多个 Luna。新增页面、主要区域、可见结构、响应式结构或视觉重做没有用户原型时必须暂停索取，不由 AI 自行设计。当前对话 Sol 亲自操控真实浏览器，按 `prototype-acceptance` 检查原型覆盖的真实消费路由、状态和窄中宽断点；浏览器反馈产生的局部样式修正默认由 Sol 直接完成，不建立 Playwright 截图基线。用户明确要求收口后才统一完成验证与交付。
- `.specs/<KEY>/` 中纯后端与混合方案以 `solution.md` 为中心；前端以用户提供并确认的 PNG、JPG、截图、HTML 原型或 Figma 为视觉依据，Sol 只在会话中维护原型映射，不生成独立设计文档。混合任务的视觉依据只从属 `solution.md`，不得改写业务、API、权限、数据或失败语义。所有路径都不保存阶段状态、哈希或历史测试标记，开始或恢复时重新读取当前请求、适用方案或原型、Git 差异和真实代码；跨会话不继承旧测试结论。
