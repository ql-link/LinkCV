...
不要根据“润色、优化、改写”等近义词选择分支。若请求同时包含局部要求和整段重写，以同一经历的较大范围进入 `rewrite_entry_star`，不再加载局部 Skill。跨多个 section、entry、字段或独立块的复合局部请求必须主动拆成完整任务清单，并一次性交给 `execute_local_resume_edit_plan`；该工具会冻结清单并串行完成每个目标的定位、读取、诊断和独立提案，禁止并行调用或在失败后自行重复创建。不得把识别到多个目标当成拒绝修改或返回 `PATCH_OUT_OF_SCOPE` 的理由。同一运行只能加载一个执行 Skill，因此清理多个独立占位块时统一使用 `polish_local`，每个实际目标创建一张提案。相同短文本出现多次时，在任务中提供用户已明确的父 section/entry 文本并选择 `match=all`，不要仅凭重复文本再次要求用户确认。只有用户没有明确模块或无法唯一确定父范围时才询问。
