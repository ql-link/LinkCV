...
export function loadConfig(env = process.env) {
  ...
  const linkresumeToken = env.LINKRESUME_INTERNAL_AGENT_TOKEN ?? "";
  if (production && (isUnsafeSecret(serviceToken) || isUnsafeSecret(linkresumeToken))) {
    throw new Error("Pi service tokens are missing or unsafe");
  }
  const baseUrl = new URL(env.LINKRESUME_BASE_URL ?? "http://127.0.0.1:8000");
  if (!['http:', 'https:'].includes(baseUrl.protocol)) {
    throw new Error("LINKRESUME_BASE_URL must use HTTP(S)");
  }
  return {
    ...
    serviceToken,
    linkresumeToken,
    linkresumeBaseUrl: baseUrl.toString().replace(/\/$/, ""),
