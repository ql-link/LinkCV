...
describe("PluginInstallDialog", () => {
  it("展示当前版本、手工安装步骤和精确下载链接", async () => {
    ...
    expect(screen.getByRole("button", { name: "下载 v0.2.0" })).toBeEnabled();
    expect(screen.getByText(new RegExp(release.sha256))).toBeInTheDocument();
  });
  ...
  it("未发布或读取失败时不显示下载链接", async () => {
    ...
    expect(await screen.findByText("当前环境尚未发布插件安装包。")).toBeInTheDocument();
  ...
  it("版本切换导致下载冲突时提示重新打开", async () => {
    ...
    render(<PluginInstallDialog onClose={vi.fn()} />);
    fireEvent.click(await screen.findByRole("button", { name: "下载 v0.2.0" }));
