...
export const InlineIcon = Node.create({
  ...
  parseHTML: () => [{
    ...
    getAttrs: (element) => element instanceof HTMLElement ? { name: element.dataset.inlineIcon || "Star" } : false,
  }],
  renderHTML: ({ HTMLAttributes }) => ["span", mergeAttributes(HTMLAttributes, { "data-inline-icon": HTMLAttributes.name ?? "Star" })],
