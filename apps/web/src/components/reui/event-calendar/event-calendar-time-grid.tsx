...
function EventCalendarTimeGrid({
  ...
  useEffect(() => {
    ...
    const scrollTo = (minutes: number) => {
      ...
      if (viewport) {
        viewport.scrollTop = Math.max(
          0,
          (minutes - startHour * 60) * pxPerMinute - 12
        )
      }
    }
    ...
    syncScrollbarGutter()
    const gutterObserver = viewport && typeof ResizeObserver !== "undefined"
    ...
    <div
      ...
      onDoubleClick={(e) => {
        if (
          ...
          settings.onSlotDoubleClick?.({ date: dayStart, allDay: true, view }, e)
        }
      }}
    >
  ...
  >(
    ...
    },
    {
      isEqual: (a, b) =>
        ...
        (a !== null &&
          b !== null &&
          Object.is(a.window[0], b.window[0]) &&
          Object.is(a.window[1], b.window[1]) &&
          a.valid === b.valid &&
          a.kind === b.kind &&
          Object.is(a.proposedStart.getTime(), b.proposedStart.getTime()) &&
          Object.is(a.proposedEnd.getTime(), b.proposedEnd.getTime())),
    }
  )
    ...
    <div
      ...
      onDoubleClick={(e) => {
        if (
          ...
          return
        const slot = slotFromPointer(e)
        settings.onSlotDoubleClick?.({ ...slot, allDay: false, view }, e)
      }}
    >
      ...
      {packedTimed.map((segment) => {
        ...
        const nestingDepth = segment.nestingDepth ?? 0
        const zIndex = segment.occurrence.event.zIndex ?? 10 + column + nestingDepth
            ...
            style={
              {
                ...
                left: nestingDepth > 0 ? `${nestingDepth * 12}px` : `${column * colPct}%`,
                width: nestingDepth > 0 ? `calc(100% - ${nestingDepth * 12}px)` : `${span * colPct}%`,
                "--ec-z": zIndex,
              } as CSSProperties
            }
            data-layout={columnCount > 1 ? "columns" : nestingDepth > 0 ? "nested" : "single"}
            data-layout-depth={nestingDepth}
            data-layout-column={column}
            data-layout-column-count={columnCount}
          >
            <EventCalendarEvent
              segment={segment}
              className={cn(
