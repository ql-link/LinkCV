...
function packTimedSegments<TData>(
  ...
  for (const cluster of clusters) {
    const strictlyContains = (outer: Working, inner: Working) =>
      outer.startMin <= inner.startMin &&
      outer.effEnd >= inner.effEnd &&
      (outer.startMin < inner.startMin || outer.effEnd > inner.effEnd)
    const containmentOnly = cluster.every((left, leftIndex) =>
      cluster.every((right, rightIndex) => {
        if (leftIndex >= rightIndex) return true
        const overlaps = left.startMin < right.effEnd && right.startMin < left.effEnd
        return !overlaps || strictlyContains(left, right) || strictlyContains(right, left)
      })
    )
    ...
    if (containmentOnly) {
      for (const item of cluster) {
        item.seg.column = 0
        item.seg.columnCount = 1
        item.seg.columnSpan = 1
        item.seg.nestingDepth = cluster.filter((candidate) =>
          strictlyContains(candidate, item)
        ).length
      }
      continue
    }
