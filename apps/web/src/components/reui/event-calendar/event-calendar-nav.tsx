...
function EventCalendarNavPrev({
  ...
  return (
    ...
    >
      <Button
        variant={nav.variant}
        size={nav.iconSize === "icon-sm" ? "icon" : nav.iconSize}
...
function EventCalendarNav({
  ...
  const defaultProps = {
    ...
    children: (
      // Shared provider is required by both the default controls and custom
      // composed nav children such as EventCalendarViewSwitcher.
      <TooltipProvider delayDuration={tooltips.delay} skipDelayDuration={tooltips.closeDelay}>
        {children ?? (
          <>
