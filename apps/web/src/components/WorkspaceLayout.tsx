...
import { BriefcaseBusiness, FileText, LayoutTemplate, LogOut } from "lucide-react";
...
export function WorkspaceSidebar({ active, email, onLogout }: WorkspaceSidebarProps) {
  return (
...
export function WorkspaceLayout({ active, children }: { active: WorkspaceSection; children: ReactNode }) {
  ...
  return (
    <div className="dashboard-shell">
      <WorkspaceSidebar active={active} email={user?.email ?? ""} onLogout={logoutAndReturn} />
      {children}
    </div>
  );
}
