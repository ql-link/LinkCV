...
def settings_env_files() -> tuple[Path, ...]:
    base = Path(os.environ.get("LINKRESUME_ENV_FILE", REPO_ROOT / ".env")).expanduser()
    if not base.is_absolute():
        base = (REPO_ROOT / base).resolve()
    configured_secret = os.environ.get("LINKRESUME_SECRET_ENV_FILE")
...
class Settings(BaseSettings):
    ...
    log_service_name: str = Field(default="linkresume", alias="LOG_SERVICE_NAME")
    ...
    mysql_database: str = Field(default="linkresume", alias="MYSQL_DATABASE")
    mysql_user: str = Field(default="linkresume", alias="MYSQL_USER")
    mysql_password: str = Field(
        default="linkresume-local-change-me",
        alias="MYSQL_PASSWORD",
    )

    jwt_secret: str = Field(
        default="linkresume-development-secret-change-me", alias="JWT_SECRET"
    )
    ...
    linkresume_internal_agent_token: SecretStr | None = Field(
        default=None, alias="LINKRESUME_INTERNAL_AGENT_TOKEN"
    )
    ...
    minio_secret_key: str = Field(
        default="linkresume-minio-local-change-me",
    ...
    rabbitmq_exchange_name: str = Field(
        default="tolink.resume.resume_import.v2",
        alias="RABBITMQ_EXCHANGE_NAME",
        min_length=1,
        max_length=255,
    )
    rabbitmq_queue: str = Field(
        default="linkresume.resume_import.worker.v2",
    ...
    def validate_production_secrets(self) -> "Settings":
        ...
        if self.log_service_name != "linkresume":
            raise ValueError("LOG_SERVICE_NAME must be linkresume")
        ...
        if self.agent_enabled:
            ...
            internal_token = (
                self.linkresume_internal_agent_token.get_secret_value()
                if self.linkresume_internal_agent_token is not None
                else None
            )
            ...
            if _is_placeholder(internal_token) or len(internal_token or "") < 32:
                invalid.append("LINKRESUME_INTERNAL_AGENT_TOKEN")
