...
from linkresume.modules.observability.context import current_context

MESSAGE_LIMIT = 2048
STACK_LIMIT = 8192
FIELD_LIMIT = 512
LOG_FILE_NAME = "linkresume.jsonl"
...
def emergency_log(message: str) -> None:
    ...
    try:
        os.write(2, f"linkresume-observability-error: {safe}\n".encode("utf-8"))
    except OSError:
        pass
...
class JsonlFileWriter:
    ...
    def _rotate_if_needed(self) -> None:
        ...
        destination = self.directory / f"linkresume.{suffix}.{uuid4().hex[:8]}.jsonl"
        self.path.replace(destination)
        self._cleanup()

    def _cleanup(self) -> None:
        cutoff = datetime.now(UTC) - timedelta(days=self.retention_days)
        for path in self.directory.glob("linkresume.*.jsonl"):
            try:
...
class StructuredLoggingHandler(logging.Handler):
    def __init__(self, emitter: StructuredLogEmitter) -> None:
        ...
        self._linkresume_structured = True
...
def configure_logging(settings: Settings) -> StructuredLogEmitter:
    ...
    for handler in list(root.handlers):
        if getattr(handler, "_linkresume_structured", False):
