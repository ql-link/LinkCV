...
def _tiptap_marked_text(node: dict[str, Any]) -> str:
    ...
    for mark in node.get("marks", []):
        ...
        if mark_type == "bold":
            ...
            value = f"[[linkresume-underline]]{value}[[/linkresume-underline]]"
            ...
            if attrs.get("color"):
                value = f"[[linkresume-color:{attrs['color']}]]{value}[[/linkresume-color]]"
            if attrs.get("fontSize"):
                value = f"[[linkresume-size:{attrs['fontSize']}]]{value}[[/linkresume-size]]"
        elif mark_type == "highlight":
            value = (
                f"[[linkresume-highlight:{attrs['color']}]]{value}"
                "[[/linkresume-highlight]]"
            )
    return value
...
def _tiptap_inline_text(node: dict[str, Any]) -> str:
    ...
    if node_type == "resumeBlockAnchor":
        ...
        return f"[[linkresume-block:{attrs['blockId']}{semantic}]]"
    ...
    if node_type == "inlineImage":
        ...
        return (
            ...
            f'"linkresume-inline-image-v2:{attrs.get("width", 72)}:'
...
def _tiptap_block_markdown(node: dict[str, Any]) -> str:
    ...
    if node_type == "avatarImage":
        ...
        return (
            ...
            f'"linkresume-avatar:{attrs.get("size", 96)}")'
        )
    if node_type == "resumeImage":
        return (
            f"![{attrs.get('alt') or '简历图片'}]({attrs['src']} "
            f'"linkresume-image:{attrs.get("width", 55)}:'
