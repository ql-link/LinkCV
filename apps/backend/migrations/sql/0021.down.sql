-- Down migration for 0021: unify document parse tasks
-- The Python revision refuses downgrade when non-resume task types exist.
...
SELECT
  ...
  task.object_name,
  task.upload_status,
  task.upload_duration_ms,
  task.parse_status,
  task.parse_duration_ms,
  task.created_at,
  task.updated_at
FROM document_parse_tasks AS task
LEFT JOIN resumes AS resume ON resume.parse_task_id = task.id
WHERE task.source_type = 'resume_import';

DROP TABLE document_parse_tasks;

ALTER TABLE resumes
  DROP INDEX uk_resumes_parse_task_id,
