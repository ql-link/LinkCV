...
- 登录后只读查看本人简历列表；详情渲染当前实时草稿（与 Web 工作台所见一致），以唯一的智能一页高清图片显示。
- 服务端临时生成文字 PDF 并转成 PNG，响应完成后不保留 PDF 或 PNG 成品。同一草稿修订的 PNG 首次下载后保存到 `wx.env.USER_DATA_PATH`，再次打开直接使用本地文件；草稿保存后修订号变化、文件损坏、退出登录或切换账号时自动更新或清理。
