...
Page({
  ...
  updateButton() {
    ...
    if (this.data.session)
      this.setData({
        sessionLabel:
          c.sessionView(this.data.session).shortTimeLabel +
          " · " +
          c.sessionView(this.data.session).modeLabel,
      });
  },
  chooseDateTime(e) {
    ...
    this.setData({
      timeEditorOpen: true,
      timeTarget: target,
      pickerDate: this.data.form[target + "Date"] || c.dateParts().date,
      pickerTime: this.data.form[target + "Time"],
      pickerTitle: target === "start" ? "选择开始时间" : "选择结束时间",
    });
  },
  closeDateTime() {
    this.setData({ timeEditorOpen: false });
  },
  acceptDateTime(e) {
    if (this.data.saving || this.data.stageLocked) return;
    const form = {
    ...
    this.setData({ form, timeEditorOpen: false, error: "" });
    this._dirty = true;
