...
- 字体与排版：两套均沿用项目系统字体。经典商务使用居中姓名、紧凑正文和蓝色细线层级；活力使用左对齐橙色姓名、橙色章节标题和主辅栏卡片层级。
- 间距与布局：经典商务保留完整 A4 单栏扫描路径，头像、姓名、职位和联系信息集中在浅蓝页眉。活力使用约 `2.35:1` 主辅栏，教育、工作和荣誉在左，技能与证书在右，卡片间距与圆角接近来源。
- 颜色与 Token：经典商务使用 `#3267e8` 与浅蓝页眉；活力使用 `#f06b32`、暖白纸张和白色内容卡片。颜色均由模板 accent token 驱动。
- 图片与资产：没有复制来源真人照片或 VIP 标识；头像使用项目现有 `/templates/avatar-cat.jpg` 安全回退，没有新增 CSS 图形、占位图或远程资源。
- 文案与内容：视觉核对使用项目虚构产品经理内容；生产模板复用已验证的 `featured-product-cn` 完整样本，不复制截图中的联系方式或履历正文。

## Focused comparison

- 经典商务聚焦核对浅蓝页眉、居中身份区、章节蓝线、公司与日期对齐。
- 活力聚焦核对顶部身份卡、个人总结分隔、左右栏比例、经历卡片和右侧能力卡片；关键细节在桌面截图中可清晰辨认，无需额外裁切。

## Comparison history

1. 首次经典商务对照未发现 P0/P1/P2，浅蓝身份区、章节线和单栏密度符合来源。
2. 首次活力对照发现 P2：章节标题继承了全宽横线，双行元信息没有完整落入同一张卡片。
3. 移除继承横线，并扩展卡片选择器覆盖日期、职位与后续项目符号；复查后标题只保留橙色左边线，经历卡片边界连续，无剩余 P0/P1/P2。

## Follow-up polish

- P3：来源使用真人肖像、VIP 徽标和少量装饰水印；为避免复制未授权素材并保持可编辑性，本实现使用安全头像回退且省略纯装饰元素。
- 真实 `/templates` 目录当前浏览器会话重定向到登录页，因此本轮视觉对照在同一 Vite 运行时的隔离渲染页完成；目录注册、模板投影和打印 HTML 由自动化测试覆盖。
