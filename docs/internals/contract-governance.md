...
| --- | --- | --- | --- |
| HTTP 路由与 schema | `apps/backend/src/linkresume/api/`、`apps/backend/src/linkresume/modules/` | Web API client、Vite 代理 | `docs/api/http-contracts.md`；业务归属见 `docs/features/`，运行结构见 `docs/internals/` |
| 浏览器导入请求 | `apps/backend/src/linkresume/modules/job_descriptions/schemas.py`、`apps/extension/src/contracts.ts` | 插件 API client、后端导入清洗 | `docs/api/http-contracts.md`、`docs/internals/extension.md` |
