...
任务计划通过内部 `tasks:plan` 接口在当前用户消息元数据中保存，限制为 1–8 项，并校验工作流、产物类型、已授权资料引用、唯一 ID 和先后依赖。Pi 逐项调用 `tasks/{taskId}:status` 更新 `running/completed/partial/blocked/failed`；服务端只接受属于同一 run 且未归属其他任务的真实提案 ID。工作流切换以当前任务为边界，读取 Skill 只提供方法，不决定用户资料授权。若计划拆成多个简历编辑任务，Pi 每次只允许当前任务提交一项编辑，已创建的提案事件按 ID 去重；单个复合任务中失败项可修正参数重试，已成功项不会重建。编辑任务的状态、错误码和摘要由实际工具结果与提案 ID 生成，不接受模型文本覆盖。模型请求若在最后一项简历编辑已完整生成提案后失败，Pi 仅在全部编辑结果均为成功、提案 ID 与数量一致且没有其他未收口任务时，依据持久化结果完成该任务并给出确定性回复；其他失败仍保持失败。运行意外结束时，FastAPI 把未收口任务标记为失败或受阻，并把该 run 已创建但未计入其他任务的提案归到当时正在执行的任务。`agent_runs.status=succeeded` 仍只表示运行和回复正常结束；每项业务结果以任务状态为准。单项提案使用基于目标与操作的稳定请求键，服务端按 run 与请求键幂等返回，批量目标部分失败时保留已创建提案 ID。
...
新简历编辑提案通过内部 `proposals:v3` 接收带 `operation_version: 3` 的类型化操作，直接作用于 canonical 节点。服务端按目标返回 `allowed_operations`，支持字段清空、单一 text run 内替换、段落或列表项删除、经历下或 bullet 后插入列表项；跨样式 run、图标和不支持的节点拒绝。新提案沿用 `resume_change_proposals`，在 `operations_json` 显式标记版本并保存有界前后预览；确认时持锁重新校验目标并对当前 canonical 快照重放，未触及节点及格式保持原样。旧版本提案仍由原有路径读取和确认。Pi 的 `execute_resume_edit_plan` 对单项和多项使用同一逐项执行器，保留部分成功的提案 ID，待确认提案不会提前改写简历。模型遗漏插入 bullet 的父锚点时，运行时只从用户原话中唯一明确的“在某经历下新增”目标补足，且必须与任务摘要匹配；随后仍由服务端验证目标和允许的操作，否则拒绝定位。

Agent 文本投影为经历结构化字段和 row 单元格正文保留各自的 canonical 节点；locator 的 `field` 标识具体经历字段。旧版 `polish_local` 每张提案只允许一个局部 operation：普通块锚点只能修改自身，section 或 entry 标题锚点可以授权其范围内一个已读取的子块，不能跨 section/entry。新旧执行器都要求复合意图先提交完整任务清单；Pi 复制并冻结清单后严格串行完成每个目标的定位、读取、诊断和提案，最多展开 20 个目标。父 section/entry 下需要清理的重复短文本可以通过 `match=all` 展开，不能脱离父范围全局匹配；单项失败写入失败状态后继续后续任务。新版单项与批量共用逐项执行器：重复提交成功项只返回既有结果，失败项可以修正目标后重试；旧版单项与批量的互斥约束仅用于历史提案兼容。`replace_target_text` 允许空字符串以清空选区或可选字段，确认时把空的可选经历字段收敛为 `null`；`delete_target` 只删除 section/entry 正文中的完整 paragraph/list item canonical 节点，不允许删除 section、entry、字段或表格单元节点，也不留下空列表项；`insert_after_target` 仍要求非空。多张范围化提案共享创建版本时，确认逻辑用当前快照和各自的目标哈希重放 operation，因其他不相交修改产生的锁版本变化不会使提案冲突，目标自身变化才返回 `TARGET_STALE`；旧快照和整篇翻译仍严格依赖原 `base_lock_version`。语义整份 locator 使用非 `resume` scope 时返回 `SCOPE_FORBIDDEN`。
