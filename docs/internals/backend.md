...
| `src/linkcv/modules/miniprogram/` | 本人简历实时草稿只读元数据、PDF 与 PNG 预览（草稿修订标识为 `draft:{lock_version}`）；校验私有图片后调用一次性 Node 渲染器，并用 PDFium 栅格化页面，不保存成品。`account_routes.py` 提供小程序专用昵称与头像读写（头像二进制仅经 `/api/miniprogram/account/avatar` 分发）；`career_routes.py` 提供求职跟进与面试安排，投递简历预览仍只读求职绑定的不可变版本 |
