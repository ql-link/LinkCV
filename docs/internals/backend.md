...
`job_applications` 保存每次投递自己的 Offer 状态与可选详情，Base、单值薪资和福利不写入共享 JD。迁移 `0052` 增加可空详情并将历史 OC/书面 Offer 状态不可逆地合并为 `received`；迁移 `0053` 把薪资区间收敛为单个 `offer_salary`，旧记录优先保留下限、仅缺少下限时取上限，并继续要求数值薪资与币种、计薪周期同时存在。
