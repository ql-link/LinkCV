...
迁移 `0064` 曾新增八份复用既有主题的岗位样本；`0065` 将这八个目录项停用，保留已创建简历的引用及快照，并新增「右栏蓝线」「浅绿素笺」「蓝色缎带」三套独立视觉模板。新模板继续使用 `CanonicalResumeDocument` 与 `TemplateDefinition`，右栏版式通过 header/main/sidebar 区域和语义插槽分配内容，其余两套为单栏；正文使用虚构示例，不嵌入来源站点的 HTML 或图片。稳定 key 已存在但名称、描述、正文或样式不一致时迁移中止，不覆盖现场数据。


迁移 `0066` 增加「时序侧标」「居中肖像」「雾蓝双栏」三套独立视觉模板，沿用现有 canonical 正文与模板定义。「雾蓝双栏」把身份及个人简介放在 header，工作与项目放在 main，教育和技能放在 sidebar；另外两套采用单栏。迁移只增添目录项，不回填已有简历；稳定 key 冲突时仍校验内容一致性。

迁移 `0067` 适量扩充上述六套视觉模板的虚构示例，补充项目背景、具体行动及成果说明。仅当正文和样式仍等于官方初始样本时更新默认正文；已自定义模板跳过，样式、启用状态及已创建简历和历史版本均保持不变。

迁移 `0068` 追加 37 套 `atlas-*` 视觉模板，官方启用目录共 51 套（不含八套已停用的岗位样本）。新增模板使用既有 header/sidebar/main/footer 区域、全语义 fallback 和虚构示例，覆盖单栏、左右侧栏、均衡双栏及页尾能力区；不改变 schema，也不回填已有简历。重复执行保留相同目录项的启用状态，稳定 key 的正文或定义冲突则拒绝覆盖。发布时先部署包含 Atlas 样式与主题识别的 Web/打印渲染器，再启用新增模板迁移，避免旧客户端把新 key 渲染为经典样式。

迁移 `0069` 追加六套 `studio-*` 模板，默认启用目录达到 57 套。版式参考预览编号 4、6、9、11、110、240，由项目独立实现，示例使用六份虚构职业经历。只插入新增目录项，重跑保留启停状态并拒绝同 key 的定义冲突，不改已有模板、简历及版本；与 Atlas 批次相同，启用前需具备对应 Web/打印主题。
...
迁移 `0070` 追加六套 MIT 授权适配的 `open-*` 模板，默认目录达到 63 套；只插入新增模板，沿用 `0069` 的重复执行、停用保留与冲突拒绝规则，无表结构变化，不改用户快照。上游固定版本及声明见[模板来源](resume-template-sources.md)。

迁移 `0071` 追加三套 `original-*` 原创模板，默认目录达到 66 套；复用既有 canonical 和区域插槽，无表结构变化。沿用新增 key 冲突拒绝、重复执行保留启停状态的策略，不覆盖已有模板、简历或版本。

迁移 `0072` 追加「横轴履历」「暖灰书页」两套原创模板，默认目录达到 68 套。工作和项目置于 header，教育与技能使用页尾区域；前者采用左侧日期轴和灰底技能横带，后者采用米白纸面与窄章节标签。沿用既有 canonical、模板快照及冲突校验，不修改已有简历和版本。
