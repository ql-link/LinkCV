...
| `src/linkcv/modules/identity/`                   | 用户模型、注册、登录、admin-login 鉴权、get_current_admin 权限依赖与双 Token 会话管理、管理端用户管理路由（列表/详情/状态变更/统计）与操作审计日志 |
...
`0008` 新增 `admin_operation_logs` 管理操作审计表，记录管理员对用户的 enable/disable 操作。字段包括 `id`（BIGINT UNSIGNED PK）、`actor_user_id`（操作人，FK → users.id）、`target_user_id`（目标用户，FK → users.id）、`action`（ENUM "disable"/"enable"）和 `created_at`。每次启用或禁用账号时通过 SQLAlchemy INSERT 写入一条记录，用于后续管理操作追溯。
