...
用户表用 BIGINT 自增主键、Argon2 密码摘要，含昵称、头像对象键、状态、管理员开关和最近登录时间；鉴权为双 Token + Redis 会话三校验，会话只存 `auth:session:{sid}`，不建 Session 表。
| 位置 | 职责 |
| --- | --- |
...
scripts/db/init_mysql.py 只允许创建名为 linkcv 的 MySQL 数据库；scripts/release/run_alembic.py 在迁移前校验环境、host、port 和数据库并输出不含密码的摘要。FastAPI 配置支持根 `.env`、显式 `LINKCV_ENV_FILE`、同名 `.local` 和进程环境覆盖。Redis 已作为鉴权会话权威存储启用，键为 `auth:session:{sid}` 与 `auth:user:{user_id}:sessions`；阿里云 OSS 仍为预留配置，图片读写只使用 MinIO。
