...
| `migrations/` | SQL-first Alembic revision；当前 head 为 `0048` |
...
`0047` 在维护窗口完成简历 canonical 一次性切流，新增表为 0。迁移预检全部 `resume_templates/resumes/resume_versions`，把旧 JSON 转为 closed `CanonicalResumeDocument/TemplateDefinition/ResumePresentation`，为历史版本补齐非空 `template_id`，把当前简历模板外键改为 `RESTRICT`，并为既有 `document_parse_tasks` 增加导入时冻结的 `selected_template_id` 与私有 `source_graph_object_name`。任何无法确定模板、无法无损表示、成功任务缺少结果简历或仍在处理的导入都会在首个 DDL 前阻断。升级后普通运行时不再双读双写；旧解析模块只保留给该迁移和离线 SQLite 导入器。

`0048` 不新增列、表或其他 schema 对象。它在首笔 JSON 更新前全量预检三张既有表：把 `0047` 已扁平化的完整 `::: left/right`、`:::: meta/trio` 容器确定性重组为 canonical `row`（pair 两格且保留 30–80 的 `left_width_percent`、meta 四格、trio 三格），拒绝嵌套、不完整或歧义容器，并保持已有内容节点/来源引用。`TemplateDefinition.avatar` 通过现有 identity/avatar slot 唯一恢复 region；官方模板采用受保护的显示和尺寸事实（administrative 108px、campus 82px、civic 94px、creative 112px，classic technical 隐藏），未知模板缺少可证明策略时 fail-closed 为隐藏且无系统 fallback。写后再次严格校验全部 JSON、确认原始 `:::` 标记为零；revision 为 forward-only，第二次预检不产生语义变化。
...
- `LINKCV_TEST_MYSQL_URL`：仅允许指向本机一次性 `linkcv` 数据库，用于从根 revision 向前升级到 `0046`、模板初始化和物理约束验证。
