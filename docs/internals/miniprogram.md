...
- `services/`：认证、资料、简历与求职 API，以及草稿修订预览缓存。
...
| `modules/miniprogram/pdf_service.py` | PDFium PNG 栅格化与预览渲染约束 |
| `modules/identity/wechat_routes.py` | 小程序登录、账号存在性、刷新和退出协议 |
| `services/auth.js`、`utils/request.js` | token 保存、刷新、重试和失效处理 |
| `services/resumePreviewCache.js` | 草稿修订预览索引和本地文件缓存 |

## 预览与缓存链

列表、详情、PDF 与 PNG 统一渲染简历当前实时草稿（`resumes.data_json/style_json`），与 Web 工作台所见一致；`pdf_version_id`/`version_id` 取值 `draft:{lock_version}`，草稿每次保存递增，客户端修订过期返回 `409 RESUME_VERSION_UNAVAILABLE`。FastAPI 复用 Web 打印核心生成智能一页 PDF，再由 PDFium 栅格化为 PNG；服务端不保存 PDF/PNG 成品。客户端按用户、简历和草稿修订缓存在 `wx.env.USER_DATA_PATH`，退出、会话失效或账号切换会清理索引和文件。

游客首页使用双列画廊卡片展示一份明确标注“内容为虚构信息”的内置示例，点击进入完全本地的静态详情，不调用账号或个人简历接口。登录后列表展示真实简历并异步预取草稿预览 PNG；页面内部 `scroll-view` 负责滚动和下拉刷新，底部导航位于滚动区域外并固定在安全区上方。真实详情在 `movable-area`/`movable-view` 中支持双指缩放与自由拖动。
...
- PDF/PNG 仅渲染当前用户本人简历的实时草稿；客户端传入的 `version_id` 只能等于当前草稿修订，不能扩大可读范围。
- 渲染并发、PDF/PNG 大小、尺寸和像素均有限制；超限或渲染失败不写缓存索引。
- 本地缓存属于可重建派生数据，账号切换和退出必须清理，不能作为服务端草稿真值。
...
`career_routes.py` 通过小程序 Bearer 依赖调用现有 interviews 应用服务，复用资源归属、乐观锁与阶段/排期幂等检查。独立表单与弹窗共用 `utils/careerEditor.js` 的保存行为。阶段成功而安排失败时保留阶段，客户端原位重试安排；排期与网页端一致，允许时间重叠，不要求 `allow_conflict` 确认。岗位内容读取这次求职保存的快照；投递简历通过求职专用 PNG 端点读取实际绑定的不可变版本，不能用当前草稿或最新版本替换，也不能由客户端指定其他版本。该预览只作临时文件使用，离开页面清理。求职场景的绑定版本语义独立于简历中心的实时草稿展示。
