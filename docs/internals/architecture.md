...
| WeChat miniprogram | `apps/miniprogram` | 原生小程序，界面采用与 Web 内部功能区同源 `--ui-*` Token 的小屏简洁单列布局与深色/中性 accent 主操作；冷启动直接展示游客可浏览的“简历”列表，其中只有一张内置“示例简历 · 内容为虚构信息”卡片，点击进入完全本地的虚构简历详情；底部仅有“简历 / 我的”两个标签页。游客首页不请求个人数据，“我的”页面在游客态展示登录入口。登录由用户主动从“我的”进入统一登录页并确认平台隐私保护指引后完成，后端自动复用已有微信账号或创建普通账号；扫码进入独立确认页确认或取消网页登录。登录后可用紧凑列表只读查看本人真实简历、以智能一页图片阅读详情，并在“我的”维护可选头像与昵称或退出登录。 |
...
- Web 登录态使用短 JWT access Cookie 与不透明 refresh Cookie；小程序使用 Bearer access 与 JSON refresh。Redis session 的 channel 阻止两端凭据混用并支持统一撤销；小程序 Bearer 只能访问 `/api/miniprogram/resumes*` 只读接口与 `/api/miniprogram/account/*` 本人资料接口，启用管理员与普通账号一样可以使用这些小程序 Bearer 业务接口，停用账号仍会被拒绝。小程序冷启动在“简历”页展示内置虚构示例卡片，可切换“我的”游客态；游客首页的示例卡片与详情不发起账号识别、登录、隐私授权或个人数据请求，“我的”游客态只保留登录入口。登录页只能由用户从“我的”主动进入，也不预先探测账号。客户端在取得新会话前要求用户勾选微信平台隐私保护指引并主动确认（未勾选时在协议区行内提示），后端自动复用已有 openid 账号，或在 `privacy_accepted=true` 时创建普通账号；登录可取消并返回原标签页。扫码确认对启用管理员同样开放，并以新的微信 code 建立独立 Web Cookie 与小程序 Bearer 会话；请求重试路径不能静默触发首次建号。`privacy_accepted` 不作为服务端持久化的同意审计记录。
- 小程序游客首页使用双列一体化画廊卡片展示一份内置示例简历（顶部占位纸张 + 虚构内容标记，点击直达静态详情），登录后列表仍采用真实预览图并异步预取正式版本 PNG 缩略图；简历首页关闭页面级滚动，使用页面内部 `scroll-view` 承担纵向滚动和下拉刷新，底部导航位于滚动区域外并固定在安全区上方，内容底部预留导航高度；下拉刷新支持静默就地更新与极速回弹。登录后的真实简历详情先查询最新手动版本（缺失时初始版本），本机不存在同版本文件时从 FastAPI 下载由共享 React-PDF 核心生成并经 PDFium 栅格化的智能一页 PNG，保存到 `wx.env.USER_DATA_PATH` 后，在当前详情页专门划分的交互视口（`movable-area` 与 `movable-view`）中直接进行双指手势缩放与自由拖拽平移查阅；服务端不保存 PDF 或 PNG 成品。退出、会话失效或账号切换会清理本地索引和文件。
- 普通 Web 登录页由 `/api/auth/capabilities` 控制：Development 可使用邮箱密码或微信扫码，Production 只显示微信小程序码；管理员密码表单只存在于 `/admin/login`。小程序只有在环境被明确识别为 `develop` 且设备本地 `linkcv_local_debug_enabled === true` 时才读取启动时生成的 `local.js`，开发者工具和真机均可用该 opt-in 直连局域网 API 8000 端口；环境识别缺失或异常、无 opt-in、读取失败或没有地址时使用 `https://linkresume.cn`，`linkcv_api_base_url` 显式 URL 覆盖优先于 `local.js` 且只在明确的 `develop` 生效；`trial/release` 忽略所有开发 storage/local.js 并使用 `https://linkresume.cn`，第三方平台扩展配置可覆盖但必须使用 HTTPS。
