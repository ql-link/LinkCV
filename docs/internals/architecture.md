...
- 鉴权采用双 Token + Redis 会话三校验：Access Token 为短期 JWT HttpOnly Cookie（`resume_session`），Refresh Token 为不透明随机串 HttpOnly Cookie（`resume_refresh`），会话只存 Redis、不落 MySQL；撤销会话即删 Redis 中的 `auth:session:{sid}`。
