...
管理员绑定 `pi_agent` 时，FastAPI 保存候选配置版本快照、创建调用记录并在请求期解密凭据，然后通过 `POST /internal/probes` 把模型、地址和 Key 临时交给 Pi。Pi 使用原生 provider 直连供应商，模型必须调用固定的 `linkresume_probe` Tool；只有探针与验证证据成功后才切换 binding，明文凭据不写入响应、日志或持久化 Run。
