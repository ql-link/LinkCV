...
工作台使用简历画布右下角的白底蓝色星芒胶囊入口打开独立右侧抽屉，页头不重复放置助手按钮；入口与抽屉共用同一 AI 标识，并在抽屉打开后保留收起状态，移动端沿用工作台侧栏的覆盖布局。选中文字后的格式浮层提供“优化表达、生成亮点、调整专业度、解释内容、继续改写”快捷入口，把所选正文作为可移除上下文带入同一个助手侧栏，不在正文上方展开第二套聊天框。`AgentPanel` 使用带副标题的简洁页头、浅蓝对话气泡、带图标的快捷操作和内嵌圆形发送按钮，自带新建对话与关闭操作；它通过 POST SSE 展示文本增量、工具状态和待确认提案，并提供重新生成。关闭或停止时取消当前请求，正常 EOF 前未收到完成、失败或取消终态会显示连接中断，而不把半条响应当作成功。提案卡显示摘要、基准版本和发生变化的简历区域，用户必须先保存当前草稿再确认应用；保存失败、提案冲突或 Agent 不可用都保留现有编辑内容。确认成功后重新读取简历和版本列表。浏览器只持有用户 Cookie，不读取 Pi 服务地址、服务 token 或模型 API Key。
...
- 标准模式由编辑器 DOM 测量产生 A4 分页边界，每页显示为具有完整白底边界和轻微阴影的独立纸张，页间只露出工作台背景空隙，不显示分隔线或页码。多行分点在页尾按自然文本行跨页续排，续页不重复圆点并保持文字缩进；其他普通块放不下时整体换页，无法拆分的单个超高块允许溢出。页面设置抽屉按页面布局、页边距、字体设置、排版设置和版本管理依次组织：上下排列、左右排列与智能一页使用同组三栏互斥的可视化选项，点击任意一项会直接切换，不需要先取消当前布局；页边距使用紧凑数值步进与页面示意，字体与正文字号归入字体设置，行距单独归入排版设置，保存版本与版本记录归入版本管理。所有设置修改后立即更新，不提供“应用设置”或批量提交按钮；预览缩放操作位于工作台页头的简历名称旁，不在页面设置中重复显示，缩放比例只在按钮、适配或 `Command/Ctrl + 滚轮` 实际改变缩放时短暂显示在屏幕中央。顶部操作区其余位置只保留页面设置、导出 PDF 和保存简历。切到左右排列后，预览自动缩放到工作区正好容纳两张 A4 与中间空隙，窗口变化时重新适配，切回上下排列恢复此前手动缩放；用户也可在页头调整当前预览缩放或恢复适配。切换前以视口中心记录当前页和页内垂直位置，重排后恢复到同一内容区域，并用可中断的方向性滑动、缩放和淡入淡出解释空间变化；系统要求减少动态效果时只恢复位置、不播放动画。排列选择只保存在当前浏览器，过渡和左右排列的自动适配缩放不写入简历快照或触发正文保存。左右排列使用隐藏的纵向副本持续测量分页并允许横向滚动。宽屏打开页面设置、版本记录或智能助手时，右侧抽屉占用对应工作区宽度，简历画布平滑收窄并向左移动；小桌面和移动端继续使用覆盖式抽屉，避免压缩简历到不可操作。系统要求减少动态效果时取消这段布局过渡。智能一页被选中时隐藏分页空隙并保持连续单页；改选上下或左右排列会同时关闭智能一页并立即进入对应排列。
