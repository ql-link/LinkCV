...
- 工作台选中同一段落内的非空文字后，可从既有浮动工具栏打开“AI 局部修改”。指令输入仍附着在选区附近；生成结果通过 ProseMirror decoration 直接嵌入原段落，删除内容使用低饱和删除线，新增内容使用蓝色下划线。该预览不修改 Tiptap JSON，因此不进入自动保存、历史版本或 PDF；同一时间只保留一条建议。用户可以继续输入要求、重新生成、放弃，或点击“应用到简历”用一次事务替换原选区并保留单步撤销。建议等待期间若原文发生变化，会标记失效并阻止应用。前端只向后端发送选中文字、指令和可选上一版建议，不发送完整简历。
