...
- 工作台选中同一段落内的非空文字后，可从既有浮动工具栏打开“AI 局部修改”。指令输入仍附着在选区附近；生成结果通过 ProseMirror decoration 直接嵌入原段落，删除内容使用低饱和删除线，新增内容使用蓝色下划线。该预览不修改 Tiptap JSON，因此不进入自动保存、历史版本或 PDF；同一时间只保留一条建议。用户可以继续输入要求、重新生成、放弃，或点击“应用到简历”用一次事务替换原选区并保留单步撤销。建议等待期间若原文发生变化，会标记失效并阻止应用。前端只向后端发送选中文字、指令和可选上一版建议，不发送完整简历；右侧智能助手仍用于跨段落或整份简历任务，不再占用选中文字的局部修改入口。
- 标准模式由编辑器 DOM 测量产生 A4 分页边界，每页显示为具有完整白底边界和轻微阴影的独立纸张，页间只露出工作台背景空隙，不显示分隔线或页码。多行分点在页尾按自然文本行跨页续排，续页不重复圆点并保持文字缩进；其他普通块放不下时整体换页，无法拆分的单个超高块允许溢出。页面设置抽屉按页面布局、页边距、字体设置、排版设置和版本管理依次组织：上下排列、左右排列与智能一页使用同组三栏互斥的可视化选项，点击任意一项会直接切换，不需要先取消当前布局；页边距使用紧凑数值步进与页面示意，字体与正文字号归入字体设置，行距单独归入排版设置，保存版本与版本记录归入版本管理。所有设置修改后立即更新，不提供“应用设置”或批量提交按钮；工作台页头不显示缩放按钮，用户通过 `Command/Ctrl + 滚轮` 调整预览缩放，比例变化时短暂显示在屏幕中央。顶部操作区其余位置只保留页面设置、导出 PDF 和保存简历。切到左右排列后，预览自动缩放到工作区正好容纳两张 A4 与中间空隙，窗口变化时重新适配，切回上下排列恢复此前手动缩放；用户仍可通过 `Command/Ctrl + 滚轮` 调整当前预览缩放。切换前以视口中心记录当前页和页内垂直位置，重排后恢复到同一内容区域，并用可中断的方向性滑动、缩放和淡入淡出解释空间变化；系统要求减少动态效果时只恢复位置、不播放动画。排列选择只保存在当前浏览器，过渡和左右排列的自动适配缩放不写入简历快照或触发正文保存。左右排列使用隐藏的纵向副本持续测量分页并允许横向滚动。宽屏打开页面设置、版本记录或智能助手时，右侧抽屉占用对应工作区宽度，简历画布平滑收窄并向左移动；小桌面和移动端继续使用覆盖式抽屉，避免压缩简历到不可操作。系统要求减少动态效果时取消这段布局过渡。智能一页被选中时隐藏分页空隙并保持连续单页；改选上下或左右排列会同时关闭智能一页并立即进入对应排列。
