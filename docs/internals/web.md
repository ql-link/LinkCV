...
面试中心使用受保护路由 `/interviews`，通过 `view=overview|schedule|records` 保留总览、排期和记录复盘视图，三个视图共享同一组前端 mock 面试数据；素材标签当前仅显示后续范围，不进入独立页面。总览流程列表示每家公司的当前阶段，而不是已完成轮次的历史归档：等待当前轮次结果时仍留在该列，确认进入下一轮后移动到下一列，已完成记录由记录复盘视图承载；流程卡片左侧显示当前轮次的面试方式或待安排状态，右侧直接显示当前轮次时间，不使用“下一场”文案。排期事件使用按公司共享的七色日历标记，首次出现时从红、橙、黄、绿、蓝、紫、灰中随机分配；记录详情可调整公司颜色，同一公司的其他阶段同步复用，并仅将该 mock 偏好保存在浏览器 `localStorage`。周排期只绘制整点横线，但允许把事件拖到整点之间的半格，落点按 30 分钟吸附并保持原持续时长，键盘方向键提供相同粒度的时间调整和按天移动；本地调整同步到当前页面的记录与右侧详情，但刷新后恢复 mock 初始值。当前实现不调用后端 API，不持久化 CRUD 结果，也不执行真实文件上传。Vite 开发模式可用 `mock=1` 在无后端、无登录态时预览页面，该预览分支在生产构建中不可达，正式路由仍使用工作区 Cookie 会话保护。
