...
导入从 `/resumes/new?mode=import` 发起，文件选择后以空白模板提交 Markdown、DOCX 或 PDF；每次提交生成新的 UUID `Idempotency-Key`，access 刷新重放保持同一个 Key。创建页在提交成功后回到主页；主页显示上传中、解析中和失败导入任务，并在存在活动任务时每 2 秒刷新列表，失败任务可删除。前端不持有 MinIO、LinkParse 或结构化模型凭据。
