...
- 选中文字后，浮动工具栏提供局部字号减小、当前数值和增大控件：未设置局部字号时以页面设置中的全局正文字号为起点，每次按 0.5pt 调整，局部值通过 Tiptap `textStyle` 与 LinkCV Markdown 标记保存并在重载后恢复，也同步映射到文字版 PDF。页面设置中的字号、行距和页边距使用带当前值的步进按钮即时调整。编辑器聚焦时不显示包围整张 A4 的浏览器默认外框，但保留光标、选区和当前行左右对齐提示。
