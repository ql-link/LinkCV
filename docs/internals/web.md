...
JD 中心页头提供“安装岗位采集插件”入口。`PluginInstallDialog.tsx` 读取当前版本，展示 Chrome/Manifest V3、发布时间、大小、SHA-256、开发者模式安装和手工更新说明；只有后端返回可用版本时才渲染同源下载链接，未发布或存储故障时不猜测地址。

管理台 `/admin/plugins` 由 `PluginReleasePanel.tsx` 承担。管理员选择单个 ZIP，客户端先检查扩展名与 20 MiB 上限，二次确认后以 multipart 上传；版本、环境和权限仍由后端作可信校验。面板只展示当前正式版本，不编译源码，也不提供历史、删除或回滚操作。
