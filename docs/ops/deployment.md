...
- 当前仓库 head `0053`；`0043` 为资料上传增加幂等、可靠排队与解析尝试字段，`0049` 为活动简历导入任务回填受理时冻结的模板定义快照，`0051` 为已登记画像结构漂移提供 forward-only 修复和发布门禁，`0052` 将历史 OC/书面 Offer 合并为统一状态并增加可选 Offer 详情字段，`0053` 将 Offer 薪资区间收敛为单值字段。
