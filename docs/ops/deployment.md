...
- Alembic revision 必须保持向前兼容上一个应用版本。当前 head `0011` 删除已停用的旧版简历内容与样式备份列；`0009` 新增管理员操作审计日志；`0008` 执行 DDL 前会按外键依赖顺序永久清空旧调用日志和模型配置，再增加 Chat 候选字段、唯一当前 binding 和日志快照，升级完成后需要管理员重新配置并设置当前 Chat 模型。新应用会双写旧 `model_name/enabled`，因此应用回滚优先保留 `0008`、`0009` schema；回滚前核对最后当前候选的旧启用镜像。Production 不自动 downgrade。
- `0011 → 0010` 只重新增加空的旧版备份列，不恢复已经删除的 JSON；如需恢复旧值或继续回滚到依赖旧格式的应用，必须使用执行 `0011` 前的外部数据库备份。不要把 schema 降级成功描述为数据已恢复。
