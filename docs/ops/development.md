...
命令先校验并创建 `linkcv`，再升级到当前 Alembic head `0031`。图片、导入源文件和插件制品读写使用 `MINIO_*` 配置；Bucket 保持私有。
