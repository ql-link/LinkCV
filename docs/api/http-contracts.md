...
`GET/PUT /api/account/preferences/career.applications.stage_visibility` 维护当前登录用户的求职看板阶段显隐。请求和响应都只包含 `{hidden_column_ids: string[]}`；无配置行时 GET 返回空数组且不写库。固定阶段 ID 只接受 `pending/screening/assessment/written_test/offer/ended`，普通面试列接受 `interview:<非空阶段名>` 且阶段名最长 100 字符；数组去重并保留首次顺序，未知字段、未知 ID、空面试名和非法类型返回 `400 INVALID_USER_PREFERENCE`。客户端不提交 `user_id` 或配置键；PUT 整体覆盖本人固定配置行并采用最后一次成功写入为准。
...
| `GET` | `/api/account/preferences/career.applications.stage_visibility` | `{hidden_column_ids}`；未保存时为空数组且不写库 |
| `PUT` | `/api/account/preferences/career.applications.stage_visibility` | 保存后的 `{hidden_column_ids}`；非法结构返回 `400 INVALID_USER_PREFERENCE` |
