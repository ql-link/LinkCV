...
新版简历编辑使用内部 `POST /internal/agent/runs/:runId/proposals:v3`。`context:read` 对可编辑目标返回 `allowed_operations`；请求的单项操作包含 `operation_version:3`、`op`、`target`、`expected_text_hash` 和可选 `new_text`，动作限定为 `clear_field`、`replace_text_range`、`delete_node`、`insert_bullet`。服务端要求提案目标与操作目标一致，复验 run 用户、任务授权、来源和目标内容；新操作写入现有提案记录，确认接口仍按当前正文重放并原子提交。旧 `proposals:v2` 和旧提案确认继续可用；Pi 新工具 `execute_resume_edit_plan` 与旧工具名均在审计白名单中。
