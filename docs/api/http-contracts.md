...
| Method | Path | 成功结果 |
| --- | --- | --- |
| `GET` | `/api/auth/admin/users` | `{items, total, page, size, pages}`，支持 `q`（ID/邮箱/昵称模糊）、`status`（启用/禁用）、`role`（admin/user）筛选和 `page/size` 分页 |
| `GET` | `/api/auth/admin/users/{userId}` | `{user}` 含 `resume_count`（简历数）和 `llm_call_count`（LLM 调用量，当前为占位值 0） |
| `PATCH` | `/api/auth/admin/users/{userId}/status` | `{ok: true, status, message}`；body 为 `{action: "disable"|"enable"}` |
| `GET` | `/api/auth/admin/stats` | `{total_users, active_users_7d, total_resumes, total_llm_calls}` 全系统概览统计 |
