...
| `POST` | `/api/agent/proposals/:proposalId/confirm` | `{resume}`；普通范围化提案更新源简历并创建 `agent` 版本；若仅有无关提案先行应用导致版本变化，服务端按目标哈希在当前快照安全重放，目标自身变化才返回 `TARGET_STALE`；旧快照与 `translate_resume` 仍要求原始版本，翻译提案返回新创建的独立简历 |
