...
Alembic `0036` 在写入前预检全部模板、当前简历和历史版本，把旧 `"1.0"` JSON 一次性转换为上述唯一契约；`0037`–`0040` 依次拆分官方编辑 Markdown、移除 typed 副本、规范区块 ID 并修正双栏插槽。`0041` 再对模板、当前简历和历史版本全量预检，把旧整篇编辑正文及跨章节残留的 `sidebar/main` 页级包装转换为无投影语义块，保留可见文字与私有用户头像，并为双栏 manifest 补齐 `profile/interests` 路由；写后重复完整校验。`0042` 恢复经典技术模板及既有快照的生产页边距并删除 `blank-cn`，历史简历依靠 `ON DELETE SET NULL` 只清空来源引用。`0043` 增加资料上传幂等和可靠调度字段，`0044`–`0046` 建立并收敛 `user_profiles`。`0047` 执行简历 canonical 一次性切流；`0048` 不改 schema，只在首笔更新前预检三张既有 JSON 表，将被 `0047` 扁平化的 section rows 确定性重组为 canonical row，并按官方受保护事实恢复 avatar 策略；非官方缺少策略时只在 identity region 唯一可证明的条件下 fail-closed 为隐藏且无 fallback。写后重新严格校验、确认原始 `:::` 标记为零，且第二次预检无语义变化。发布顺序为停止旧写入、备份、从 `0046` 按顺序执行 `0047`–`0048`、验证后启动新应用；失败时依赖备份恢复，不执行 downgrade。
