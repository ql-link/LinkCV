...
- Web `features/account/`：昵称、头像、最近助手会话，以及跨简历共享的个人画像查看与编辑；助手会话沿用服务端置顶优先、最近更新次之的顺序。
...
| 账号 | `modules/identity/account_routes.py` | 本人资料、头像、个人画像，以及账号级产品偏好的读取和保存 |
...
- 用户中心的加载、保存和上传异常统一在页面中上方展示，不使用占据业务卡片位置的局部错误块。
...
- `GET/PUT /api/account/preferences/career.applications.stage_visibility` 只维护本人求职看板的阶段显隐；当前配置值仅包含 `hidden_column_ids`，无记录时返回空数组且不写库。
...
`users` 是账号、状态、管理员标记、昵称、头像对象键和微信绑定信息的权威表。`user_profiles` 与用户一对一，保存城市、工作性质、薪资、工作经验、教育背景和技能成果，不复制到简历内容。`user_preferences` 以 `(user_id, preference_key)` 为主键保存经过服务端类型校验的账号级产品偏好；当前只允许求职看板阶段显隐键。Redis 保存可撤销 session；对象存储保存头像二进制。业务模块不能自行解析 Cookie/Bearer token 或复制用户状态。
...
6. 阶段显隐保存由服务端根据当前登录用户和固定配置键插入或覆盖；客户端不提交用户 ID 或任意配置键。
7. 管理员启停用户只改变账号状态，其他模块在鉴权依赖处统一阻止禁用账号继续访问。
