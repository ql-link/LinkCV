...
- `/assistant`：新建对话入口；`/assistant/:sessionId`：可刷新、可直接访问的独立会话地址。页面同时提供会话列表、快捷任务、上下文选择、对话、停止和提案确认。置顶会话显示在独立的 `Pinned` 分组，未置顶会话显示在“最近对话”；没有置顶会话时不渲染空的 `Pinned` 分组。Pin、Unpin、Rename 和 Delete 操作共用三点菜单；菜单仅在上方有足够列表空间时向上展开，否则向下展开并由滚动列表预留完整显示高度。输入框键入 `@` 会在上方打开本人资料与简历的搜索列表，结果按“简历”和“资料”分组，`@` 后文字按资料名或简历名的前缀同时匹配，`Tab` 默认选择首项。选中结果在编辑区内成为不可拆分的文件引用单元，普通文字可以插在引用前后；发送后的用户消息继续在原位置显示内联文件单元，不显示 `@文件名` 占位文本或气泡下方的重复文件标签，提交协议仍使用结构化上下文引用。
...
- Agent Markdown 统一支持标题、段落、水平分隔线、表格、有序与无序列表、引用、强调、删除线、链接以及围栏/行内代码；一级至三级标题分别映射为消息内的 `h2`、`h3`、`h4`，字号按层级递减且始终大于所在消息正文。原始 HTML 不执行，远程图片只显示文字占位，任务列表、脚注和 Mermaid 等扩展语法不在普通消息边界执行。
- 独立助手保留专用的既有字体，不跟随其他业务页面的中文内容字体切换；会话导航、按钮、选择器和上下文标签继续使用既有 UI 字体栈。字体只影响浏览器呈现，不改变消息正文、Markdown 或上下文引用的保存值。
- 独立助手成功完成一轮发送后清空输入框内的草稿与已选引用；失败或停止时继续保留它们，以便用户重试或调整。
