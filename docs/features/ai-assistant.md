...
LinkResume AI 助手提供独立对话工作区和简历编辑器侧栏，允许用户组合简历、历史版本、岗位、求职进程、面试复盘和资料集作为上下文，执行分析、澄清和简历修改建议。它不绕过用户确认直接改写简历，也不向用户暴露模型密钥或 Pi 服务。
...
- 独立助手成功完成一轮发送后清空输入框内的草稿与已选引用；失败或停止时继续保留它们，以便用户重试或调整。请求、生成和提案操作失败复用页面根层上方居中的统一反馈浮层，局部字段校验、上下文选择状态和带恢复操作的会话约束仍留在对应区域。
...
5. 用户确认提案后，FastAPI 重新读取目标和简历锁版本，校验 canonical 文档、presentation、模板快照与私有图片导出契约，验证通过才调用简历应用服务写入并返回对应 `LayoutPlan`。
