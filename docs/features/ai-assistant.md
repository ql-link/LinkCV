...
- `/assistant`：新建对话入口；`/assistant/:sessionId`：可刷新、可直接访问的独立会话地址。页面同时提供会话列表、快捷任务、上下文选择、对话、停止和提案确认。会话列表的 Pin、Rename 和 Delete 操作共用三点菜单；菜单仅在上方有足够列表空间时向上展开，否则向下展开并由滚动列表预留完整显示高度。输入框键入 `@` 会在上方打开本人资料与简历的搜索列表，结果按“简历”和“资料”分组，`@` 后文字按资料名或简历名的前缀同时匹配，`Tab` 默认选择首项。选中结果在编辑区内成为不可拆分的文件引用单元，普通文字可以插在引用前后；发送后的用户消息继续在原位置显示内联文件单元，不显示 `@文件名` 占位文本或气泡下方的重复文件标签，提交协议仍使用结构化上下文引用。
