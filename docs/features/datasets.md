...
上传选择、冲突处理和上传队列仅在用户打开上传入口或向文件夹页面拖入文件后按需加载；页面级重试标识在弹窗关闭后仍会保留，避免网络结果不明确时重复创建文件。文件预览弹窗从所选文件的详情栏按需打开，Markdown 渲染器只在预览实际拿到文本内容后加载，不进入资料库首屏依赖。

## 用户入口

Web `/datasets` 在主体首页采用与 macOS 访达一致的纯粹网格布局，仅展示分类文件夹，已归入文件夹的资料仅在对应文件夹内展示（文件卡片按 Markdown、Word、PDF、TXT、音频等类型显示对应应用图标和格式标识；文件夹采用经典浅天蓝 macOS 立体平滑折耳矢量图标呈现，放大文件夹图标并对齐网格基线，页头提供「新建文件夹」操作入口，同时网格末尾提供同轮廓的柔和浅蓝底色幽灵卡片并支持等比放大动效）。AI 助手侧栏的“资料库”入口复用同一资料库组件，在 `/assistant` 工作区内切换展示，不跳转路由；内嵌状态中的文件夹切换也只更新组件内状态。独立资料库路由仍深度整合浏览器历史记录（`/datasets?folder=<id>`），无论通过页面「返回全部资料」还是通过鼠标侧键、浏览器前进/后退手势返回，都能准确在文件夹与资料库首页之间导航，不发生越级跳出。当存在文件夹或资料时，不再显示侵入式引导提示。首页不提供上传入口，拖入文件只提示先进入文件夹。上传必须从文件夹内发起；上传弹窗采用规范化 Dialog，不提供文件夹选择器，固定上传至当前所在文件夹，文本色彩显式绑定语义 Token 确保清晰对比度，文件夹内支持全屏拖拽文件直接上传至当前文件夹，批次开始后固定上传目标；文件夹内页标题不显示图标，上方采用与求职中心详情页一致的面包屑导航，左箭头文字按钮返回全部资料，并展示当前文件夹名称。文件区右侧常驻详情栏，未选择文件时展示中性选择提示；点击卡片只改变当前选择，详情栏展示完整文件名、格式、大小、上传日期和真实的面试关联，文档从「查看文件」打开预览，音视频从同一入口下载。当前接口没有简历关联字段，因此详情栏不虚构简历关联。用户点击文件夹可下钻查看其专属资料并在该分类下上传，支持创建、重命名、确认后永久删除文件夹与单项/批量移动归类。进入文件夹后支持批量操作，底部提供平滑升起的浮动胶囊操作栏（含已选项数、移动至分类与删除）。后台保留 `queued`、`processing`、`succeeded`、`failed` 四种解析状态；音视频上传成功后不显示等待解析状态。支持重命名、失败重试、移动分类、单个删除和批量删除。未完成或失败的文档不开放内容读取；读取失败保留重试入口。成功内容使用 Markdown 预览，按需渲染 Mermaid，并过滤解析服务写入的分页、表格控制标记。AI 助手只使用用户在当轮显式选择的已完成资料。

服务端接受的文件直接进入正式列表；未接受项及其他操作异常通过页面根层上方居中的统一浮层反馈原因，并在 3 秒后自动消失。提示包含恢复操作时，文案和按钮分居卡片左右；已接受资料的列表同步失败保留“重新刷新”入口，且刷新按钮固定在卡片右侧。

## 预览与同名上传

选择解析成功的文档后，通过右侧详情栏的「查看文件」打开只读预览弹窗，支持 Markdown 表格、换行和 Mermaid 图表。关闭后回到原文件位置，不提供正文编辑、自动保存或独立编辑页。已保存正文保持可读，更新资料通过同名上传替换完成。
...
| Web | `features/datasets/` | macOS 风格文件夹、按类型区分的文件卡片、持久详情栏、移动弹窗、自动上传队列、批量管理和 Markdown/Mermaid 预览 |
