...
- `document_parse_tasks` 保存简历导入和资料集共用的上传/解析状态。简历导入冻结 `selected_template_id`，并把确定性的 `SourceGraph` 保存到私有对象；资料集任务额外使用 `queued`、派发时间和尝试版本完成 MQ 恢复。PDF/DOCX 由 LinkParse 转换文字并可附带有界布局提示，Markdown 在 Worker 本地转换。
...
2. Worker 根据格式选择本地 Markdown 转换或 LinkParse，把解析块与可用坐标统一成稳定 `source_id` 的 `SourceGraph`。LLM 只返回稀疏语义标注，未标注源块由确定性组合器按来源顺序保留，因此不会因模型未覆盖全部块而失败，也不会生成“未分类内容”。
3. 组合器把 canonical 内容注入任务冻结的模板，编译并校验 `LayoutPlan` 后，在同一事务创建简历、initial 版本并完成任务；模板、来源图或快照任一不一致时不创建半成品。
