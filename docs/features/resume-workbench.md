...
选中文字后，浮动工具栏按粗体、删除线、斜体、下划线、链接、字体、本行类型、对齐方式排列，块级的两项与前面的内联格式之间用竖线分隔，下拉统一向下展开。「链接」为选区填写网址，选中已有链接时可取消，并沿用简历规则拒绝把邮箱或 `mailto:` 设为链接；「本行类型」在正文与标题 1–3 之间切换当前块，二级标题是章节锚点，改成其他层级会让该块失去章节语义；「对齐方式」设置当前块的左 / 中 / 右对齐。「字体」弹层把字号、字体颜色和背景颜色收在一处：字号沿用 6–48 pt、0.5 pt 步进的数值控制，未覆盖时显示模板或全局排版的实际数值，混合选区显示“混合”并以首字字号为基准统一调整；字体颜色以文字颜色本身渲染色板，背景颜色为两行色块，两者都用斜杠表示“取消”，并可一键恢复默认。列表、缩进和「AI 修改」不出现在选中弹窗，撤销 / 重做仍是编辑器顶部左侧的常驻按钮（无可撤销内容时置灰）。浮动工具栏随画布缩小同步变小，画布放大到 100% 以上时保持基准尺寸，不再跟着放大。支持单字、跨段落、姓名、章节标题、联系方式、经历字段、列表及分栏正文；局部字号随当前快照和正式版本保存，并用于模板切换、只读预览、分享与 PDF。
