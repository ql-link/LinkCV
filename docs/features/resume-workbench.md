...
移动端分享页把固定 210mm 的 A4 DOM 按滚动容器实际内容宽度整体缩放（CSS `zoom`，不支持时回退 `transform: scale` 并以缩放后高度占位）；`text-size-adjust` 抑制浏览器文字放大，微信 webview 再通过官方 `WeixinJSBridge` 的 `setFontSizeCallback` 把字号恢复标准档，避免字体放大撑破固定纸面造成重排变形。屏幕缩放只服务小屏预览，打印媒体下解除缩放仍输出原始 A4 文字版。响应中的简历资产地址已改写为分享域地址，匿名访问者可直接读取内嵌图片（见 HTTP 契约）。
