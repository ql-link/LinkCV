...
选中文字后，浮动工具栏提供局部字号数值和增大 / 减小箭头（6–48 pt，0.5 pt 步进，无下拉列表），并保留加粗、斜体、下划线、文字颜色和高亮颜色；列表、缩进、恢复默认字号和「AI 修改」都不再出现在选中弹窗，撤销 / 重做改为编辑器顶部左侧的常驻按钮（无可撤销内容时置灰）。浮动工具栏随画布缩小同步变小，画布放大到 100% 以上时保持基准尺寸，不再跟着放大。支持单字、跨段落、姓名、章节标题、联系方式、经历字段、列表及分栏正文；未覆盖字号时显示模板或全局排版的实际数值；混合选区显示“混合”，箭头以首字字号为基准统一调整；局部字号随当前快照和正式版本保存，并用于模板切换、只读预览、分享与 PDF。
