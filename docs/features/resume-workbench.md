...
命令面板会用强调色标出光标所在行真实的块类型（正文、标题 1–3、列表、左右分栏）；左右分栏支持两侧独立输入，光标位于分栏行内时再点一次“左右分栏”即可取消分栏、合并回普通正文，也能用退格删除已经清空的分栏行；右键分栏行可切换为 3 栏或 4 栏；3/4 栏默认等分，拖动栏间分隔线可以调整相邻两栏的宽度占比，双击分隔线恢复等分，改栏数后重新等分；移除比例调整控件后，已有简历的分栏比例仍保留在正文快照中，保存、模板切换和导出继续沿用既有排版规则。分栏与其他区块中的图片、图标等可选中叶子节点支持鼠标点选为节点选中，便于剪切复制；按住从这类节点拖动时选区锚点固定在节点自身边界，行内原子可按拖动方向扩展覆盖相邻文字，块级原子拖动时保持节点选中、不扩展进周边文字，从文字拖入这类节点且端点落在其上时选区恰好覆盖该节点。
