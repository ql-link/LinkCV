...
选中文字后，浮动工具栏提供局部字号数值和增大 / 减小箭头（6–48 pt，0.5 pt 步进，无下拉列表）和恢复默认。支持单字、跨段落、姓名、章节标题、联系方式、经历字段、列表及分栏正文；未覆盖字号时显示模板或全局排版的实际数值；混合选区显示“混合”，箭头以首字字号为基准统一调整；恢复默认只移除字号覆盖，保留颜色、加粗等其他格式，重新跟随模板或全局排版。局部字号随当前快照和正式版本保存，并用于模板切换、只读预览、分享与 PDF。
