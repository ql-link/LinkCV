...
2. 编辑器把领域快照转换为可编辑视图，正文和页面设置先同步更新前端 Store 并立即作用于画布；未写回数据库的修改按用户和简历隔离保存为浏览器本地草稿，在重新载入相同 `lock_version` 时恢复。数据库自动保存按 10 秒固定节拍检查脏状态并批量发送 `base_lock_version`，不因每个字符或单次页面设置调整单独请求；保存请求串行执行，成功后接续服务端新锁版本，顶部“保存简历”仍可立即写回。
