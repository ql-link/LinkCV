...
2. 编辑器根据当前 `LayoutPlan` 把 canonical 内容投影为可编辑视图。页面设置面板只提供页面排列、上下与左右边距和正文排版；章节语义仍随 canonical 内容保存，但不在该面板中展示或手动调整。正文和页面设置先同步更新前端 Store 并立即作用于画布，边距预览不等待数据库保存完成；未写回数据库的修改按用户和简历隔离保存为浏览器本地草稿，在重新载入相同 `lock_version` 时恢复。数据库自动保存按 10 秒固定节拍检查脏状态，先反投影为 canonical 文档再串行发送 `base_lock_version`，不因每个字符或单次页面设置调整单独请求；成功后接续服务端新锁版本，顶部“保存简历”仍可立即写回。
