...
2. “筛选中”不设用户自定义名称或轮次，并直接等待筛选结果；收到公司明确通知后，用户再添加已经确认的笔试/测评或面试阶段，同时填写日期时间、时长、方式以及链接或地点。面试展示名称由用户按公司实际流程自由填写，例如“一面”“二面”或“HR 面”，后端不把名称收敛为固定枚举；内部数字轮次由客户端按当前面试轮次自动递增并继续用于状态机和排期关联，不要求用户填写。客户端先按后端状态机推进阶段，再通过既有排期接口创建对应场次；若排期创建失败，已推进的阶段保留为等待安排，用户可从安排时间入口重试。
